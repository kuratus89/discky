...
void* getVecElement(Discky* discky ,const Vec* vector ,const int x){
    if((vector->capacity<=x)||(x<0))callErrorHandle(discky , ERROR_INTERNAL , "internal error : invalid vector index");
    return ((char*)vector->vector)+ (x*vector->size);
}

void resizeVecCapacity(Discky* discky , Vec* vector ,const int x){
